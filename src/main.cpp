...
int main()
{
	...
	while (dx12w::update_window())
	{
		...
		// IK
		{
			...
			auto pos = XMVector3Transform(XMLoadFloat3(&pmx_bone[bone_name_to_bone_index[L"右足首"]].position), model.bone[bone_name_to_bone_index[L"右足ＩＫ"]]);
			XMFLOAT3 float3;
			XMStoreFloat3(&float3, pos);
			solve_CCDIK(model.bone, bone_name_to_bone_index[L"右足ＩＫ"], pmx_bone, float3, to_children_bone_index, ik_rotation_num, is_ideal_rotation, is_residual);
		}
		{
			/*
			auto [rotation, transform] = calc_rotation_and_transfom_matrix(model.bone, bone_name_to_bone_motion_data, bone_name_to_bone_index, frame_num, to_children_bone_index, L"右つま先ＩＫ");
			auto [hoge, fuga] = calc_rotation_and_transfom_matrix(model.bone, bone_name_to_bone_motion_data, bone_name_to_bone_index, frame_num, to_children_bone_index, L"右足ＩＫ");
			auto rot = XMMatrixTranslationFromVector(-XMLoadFloat3(&pmx_bone[bone_name_to_bone_index[L"右足首"]].position)) *
				hoge *
				XMMatrixTranslationFromVector(XMLoadFloat3(&pmx_bone[bone_name_to_bone_index[L"右足首"]].position));
				*/
			auto pos = XMVector3Transform(XMLoadFloat3(&pmx_bone[bone_name_to_bone_index[L"右つま先"]].position), model.bone[bone_name_to_bone_index[L"右つま先ＩＫ"]]);
			XMFLOAT3 float3;
			XMStoreFloat3(&float3, pos);
			solve_CCDIK(model.bone, bone_name_to_bone_index[L"右つま先ＩＫ"], pmx_bone, float3, to_children_bone_index, ik_rotation_num, is_ideal_rotation, is_residual);
		}
		{
			//auto [rotation, transform] = calc_rotation_and_transfom_matrix(model.bone, bone_name_to_bone_motion_data, bone_name_to_bone_index, frame_num, to_children_bone_index, L"左足ＩＫ");
			auto pos = XMVector3Transform(XMLoadFloat3(&pmx_bone[bone_name_to_bone_index[L"左足首"]].position), model.bone[bone_name_to_bone_index[L"左足ＩＫ"]]);
			XMFLOAT3 float3;
			XMStoreFloat3(&float3, pos);
			solve_CCDIK(model.bone, bone_name_to_bone_index[L"左足ＩＫ"], pmx_bone, float3, to_children_bone_index, ik_rotation_num, is_ideal_rotation, is_residual);
		}
		{
			/*
			auto [rotation, transform] = calc_rotation_and_transfom_matrix(model.bone, bone_name_to_bone_motion_data, bone_name_to_bone_index, frame_num, to_children_bone_index, L"左つま先ＩＫ");
			auto [hoge, fuga] = calc_rotation_and_transfom_matrix(model.bone, bone_name_to_bone_motion_data, bone_name_to_bone_index, frame_num, to_children_bone_index, L"左足ＩＫ");
			auto rot = XMMatrixTranslationFromVector(-XMLoadFloat3(&pmx_bone[bone_name_to_bone_index[L"左足首"]].position)) *
				hoge *
				XMMatrixTranslationFromVector(XMLoadFloat3(&pmx_bone[bone_name_to_bone_index[L"左足首"]].position));
				*/
			auto pos = XMVector3Transform(XMLoadFloat3(&pmx_bone[bone_name_to_bone_index[L"左つま先"]].position), model.bone[bone_name_to_bone_index[L"左つま先ＩＫ"]]);
			XMFLOAT3 float3;
			XMStoreFloat3(&float3, pos);
			solve_CCDIK(model.bone, bone_name_to_bone_index[L"左つま先ＩＫ"], pmx_bone, float3, to_children_bone_index, ik_rotation_num, is_ideal_rotation, is_residual);
		}
