...
int main()
{
	...
	while (dx12w::update_window())
	{
		...
		// IK
		{
			auto transform = calc_transfom_matrix(model.bone, bone_name_to_bone_motion_data, bone_name_to_bone_index, frame_num, to_children_bone_index, L"右足ＩＫ");
			auto pos = XMVector3Transform(XMLoadFloat3(&pmx_bone[bone_name_to_bone_index[L"右足首"]].position), transform);
			XMFLOAT3 float3;
		...
		/*
		{
			auto transform = calc_transfom_matrix(model.bone, bone_name_to_bone_motion_data, bone_name_to_bone_index, frame_num, to_children_bone_index, L"右つま先ＩＫ");
			auto pos = XMVector3Transform(XMLoadFloat3(&pmx_bone[bone_name_to_bone_index[L"右つま先"]].position), transform);
			XMFLOAT3 float3;
			XMStoreFloat3(&float3, pos);
		...
		*/
		{
			auto transform = calc_transfom_matrix(model.bone, bone_name_to_bone_motion_data, bone_name_to_bone_index, frame_num, to_children_bone_index, L"左足ＩＫ");
			auto pos = XMVector3Transform(XMLoadFloat3(&pmx_bone[bone_name_to_bone_index[L"左足首"]].position), transform);
			XMFLOAT3 float3;
			XMStoreFloat3(&float3, pos);
		...
		/*
		{
			auto transform = calc_transfom_matrix(model.bone, bone_name_to_bone_motion_data, bone_name_to_bone_index, frame_num, to_children_bone_index, L"左つま先ＩＫ");
			auto pos = XMVector3Transform(XMLoadFloat3(&pmx_bone[bone_name_to_bone_index[L"左つま先"]].position), transform);
			XMFLOAT3 float3;
			XMStoreFloat3(&float3, pos);
