#pragma once
#include"../external/mmd-loader/mmdl/pmx/loader_traits.hpp"
#include"../external/mmd-loader/mmdl/vmd/loader_traits.hpp"
#include"../external/mmd-loader/mmdl/vpd/loader_traits.hpp"
#include"struct.hpp"
...
namespace mmdl
{
	...
	{
		template<>
		struct header_traits<model_header_data>
		{
			static model_header_data construct(header_buffer const& buffer)
			{
		...
		template<>
		struct info_traits<model_info_data>
		{
			using char_type = char;

			template<std::size_t CharBufferSize>
			static model_info_data construct(info_buffer<char_type, CharBufferSize> const&) {
				return {};
			}
		};

		template<>
		struct vertex_traits<std::vector<model_vertex_data>>
		{
			// サイズを指定して構築
			...
			// 要素を末尾に追加
			static void emplace_back(std::vector<model_vertex_data>& vertex, vertex_buffer const& buffer, std::uint8_t additional_uv_num)
			{
				model_vertex_data v{};
		...
		template<>
		struct surface_traits<std::vector<index>>
		{
			// サイズを指定して構築
		...
		template<>
		struct texture_path_traits<std::vector<std::wstring>>
		{
		...
		template<>
		struct material_traits<std::pair<std::vector<material_data>, std::vector<material_data_2>>>
		{
			using char_type = wchar_t;
			...
			// 要素を追加
			template<std::size_t CharBufferSize>
			static void emplace_back(auto& mat, material_buffer<char_type, CharBufferSize> const& buffer)
			{
		...
		template<>
		struct bone_traits<std::vector<model_bone_data>>
		{
			using char_type = wchar_t;
			...
			static void emplace_back(std::vector<model_bone_data>& bone, bone_buffer<char_type, CharBufferSize, IKLinkBufferSize> const& buffer)
			{
				model_bone_data b{};
		...
		template<>
		struct morph_traits<std::pair<std::vector<vertex_morph>, std::vector<group_morph>>>
		{
			using char_type = wchar_t;
			...
			static void emplace_back(std::pair<std::vector<vertex_morph>, std::vector<group_morph>>& result, morph_buffer<char_type, CharBufferNum, MorphDataNum> const& buffer)
			{
				...
				if (buffer.morph_type == 1)
				{
					...
					{
						auto const& v = std::get< morph_buffer<char_type, CharBufferNum, MorphDataNum>::VERTEX_MORPH_INDEX>(buffer.morph_data[i]);
				...
				else if (buffer.morph_type == 0)
				{
					...
					for (std::size_t i = 0; i < static_cast<std::size_t>(buffer.morph_data_num); i++)
					{
						auto g = std::get<morph_buffer<char_type, CharBufferNum, MorphDataNum>::GROUP_MORPH_INDEX>(buffer.morph_data[i]);
		...
		template<>
		struct rigidbody_traits<std::vector<rigidbody>>
		{
			using char_type = wchar_t;
			...
			static void emplace_back(std::vector<rigidbody>& result, rigidbody_buffer<char_type, CharBufferNum> const& buffer)
			{
				rigidbody r{};
		...
		template<>
		struct joint_traits<std::vector<joint>>
		{
			using char_type = wchar_t;
			...
			// 剛体を追加
			template<std::size_t CharBufferNum>
			static void emplace_back(std::vector<joint>& result, joint_buffer<char_type, CharBufferNum> const& buffer)
			{
				joint j{};
	...
	{
		template<>
		struct header_traits<vmd_header>
		{
			static vmd_header construct(header_buffer const& buffer) {
				return { buffer.frame_data_num };
			}
		};

		template<>
		struct frame_data_traits<std::vector<vmd_frame_data>>
		{
			...
			static void emplace_back(std::vector<vmd_frame_data>& frame_data, frame_buffer const& buffer)
			{
		...
		template<>
		struct morph_data_traits<std::vector<::vmd_morph_data>>
		{
			...
			static void emplace_back(std::vector<::vmd_morph_data>& result, morph_buffer& buffer)
			{
				auto name = ansi_to_utf16(buffer.name.data());
	...
	{
		template<>
		struct vpd_data_traits<std::vector<vpd_data>>
		{
			...
			static void emplace_back(std::vector<vpd_data>& vpd_data, buffer const& buffer)
			{
				auto name = ansi_to_utf16({ buffer.name.data() });
